...
document.addEventListener('DOMContentLoaded', () => {   // DOM（HTMLのツリー構造）の完成後に処理するイベントリスナー
  ...
  // 盤面初期化
  function resetBoardFromInitial() {
    ...
    for (let side of ["sente", "gote"]) {
      for (let piece of ["FU", "KY", "KE", "GI", "KI", "KA", "HI"]) {
        hand[side][piece] = 0;
      }
    }
  ...
  function drawFullBoard() {
    ...
    for (let row = 0; row < 9; row++) {
      for (let col = 0; col < 9; col++) {
        ...
        cell.style = `border:1px solid #000; width:70px; height:77px; display:flex; align-items:center; justify-content:center; background:#cc841d; box-sizing:border-box;`;
        cell.dataset.row = row;
        cell.dataset.col = col;
        ...
        if (piece) {
          ...
          const prefix = piece.owner === 'gote' ? 'r_' : '';
          img.src = `/assets/koma/${prefix}${piece.type}.png`;
          img.style.width = '100%';
          img.style.height = '100%';
          img.style.objectFit = 'contain';
          cell.appendChild(img);
  ...
  function drawHands() {
    let senteDiv = document.getElementById("sente-hand");
    let goteDiv = document.getElementById("gote-hand");

    senteDiv.innerHTML = "";
    goteDiv.innerHTML = "";

    for (let piece of ["FU", "KY", "KE", "GI", "KI", "KA", "HI"]) {
      if (hand.sente[piece] > 0) {
        let img = document.createElement("img");
        img.src = `images/${piece}.png`;
        img.alt = piece;
        senteDiv.appendChild(img);
        senteDiv.appendChild(document.createTextNode("x" + hand.sente[piece]));
      }
      if (hand.gote[piece] > 0) {
        let img = document.createElement("img");
        img.src = `images/r_${piece}.png`;
        img.alt = piece;
        goteDiv.appendChild(img);
        goteDiv.appendChild(document.createTextNode("x" + hand.gote[piece]));
      }
    }
  }

  // 実際の一手適用ロジック (盤面・持ち駒両方更新)
  function applyMove(move, color) {
    const turn = color === 0 ? 'sente' : 'gote';

    if (move.drop) {
      const toX = 9 - move.to.x;
      const toY = move.to.y - 1;
      const piece = move.piece;

      boardState[toY][toX] = { type: piece.toLowerCase(), owner: turn };
      hand[turn][piece] -= 1;
    } else {
      ...
      if (move.capture) {
        ...
        if (capturedPiece.startsWith('+')) {
          capturedPiece = capturedPiece.substring(1);
        }
        let opponent = turn === 'sente' ? 'gote' : 'sente';
        hand[turn][capturedPiece] += 1;
      }

      // 成り処理
      let movingPiece = boardState[fromY][fromX];
      if (move.promote) {
        movingPiece.type = "+" + movingPiece.type;
      }

      boardState[toY][toX] = movingPiece;
  ...
  // 複数手の適用（巻き戻し・早送り用）
  function applyJKFMoves(upTo) {
    resetBoardFromInitial();
    for (let i = 1; i <= upTo; i++) {
      const moveObj = jkfMoves[i];
      if (!moveObj) continue;
      applyMove(moveObj.move, moveObj.move.color);
    }
  }
